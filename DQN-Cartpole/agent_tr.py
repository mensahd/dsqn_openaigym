...
class Agent:
    ...
    def step(self, state, action, reward, next_state, done, loss_hist=None):
        ...
        if self.t_step == 0:
            ...
            if len(self.memory) > self.batch_size:
                ...
                self.optimize_model(experiences, loss_hist)

        # For training purposes, create sample of reasonable environment states
        if self.visualize_environment_states:
            self.state_list = np.vstack((self.state_list, state))
            if self.t_step_total % 1000 == 0:
                # Plot the environment state to the histogram
                for i in range(4):
                    self.plot_hist(self.state_list[:, i], i)

    def optimize_model(self, experiences, loss_hist=None):
    ...
    def train_agent(self):
        ...
        for i_episode in range(1, self.num_episodes + 1):
            ...
            # episode_loss = np.mean(current_loss)
    ...
    def create_training_images(self, frames, current_frame_number, csv_writer, img_name, state, total_nr_samples, next_frame_number):
        ...
        return next_frame_number, total_nr_samples
