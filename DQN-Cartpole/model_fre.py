...
class DSNN(nn.Module):
    ...
    def forward(self, inputs, loihi=False):
        ...
        if self.scaler:
            ...
            transformed_input = torch.maximum(transformed_input, torch.zeros(transformed_input.size()))
        ...
        if self.add_bias:
            transformed_input = torch.hstack([transformed_input,  (torch.ones((transformed_input.size(dim=0),1))*0.5).to(device)])
        ...
        spike_train= []
        for state_array in transformed_input:
            ...
            for state_value in state_array:
                ...
                zero_adjusted_state_value = min(np.round(state_value*self.simulation_time * ((self.simulation_time-2)/self.simulation_time)+2), self.simulation_time)
        ...
        #np.dsplit( preliminary, divider_array)
        #firing_rate = transformed_input
        #interval = self.simulation_time / firing_rate
        #indices = np.arange(0, len(spike_train), interval)
        #np.put_along_axis(spike_train, indices, 1, axis=1)

        reconstructed_values = np.sum(np.array(spike_train),axis=0)/self.simulation_time
